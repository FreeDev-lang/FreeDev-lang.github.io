  ...
  const linkSections = [
    {
      ...
      links: [
        ...
        { text: "Featured", path: '/products?featured=true', icon: null },
        { text: "Chairs", path: '/products?category=CHAIRS', icon: null },
        { text: "Tables", path: '/products?category=TABLES', icon: null },
      ]
    },
    {
      title: "WEBSITE?",
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
          <div>
            ...
            <p className="max-w-[410px] mt-6 text-sm">Welcome to gocart, your ultimate destination for modern furniture with AR experience. Transform your space with style and innovation — all in one place.</p>
            <div className="flex items-center gap-3 mt-5">
              {socialIcons.map((item, i) => (
                <Link to={item.link} key={i} className="flex items-center justify-center w-10 h-10 bg-slate-100 hover:scale-105 hover:border border-slate-300 transition rounded-full">
                  <item.icon />
                </Link>
              ))}
            </div>
